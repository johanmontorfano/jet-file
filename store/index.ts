...
export class JetStore {
    ...
    editKey(key: string, value: any) {
        const keyHash = JetStore.hashing(key);
    ...
    /** Read a key */
    readKey<T = any>(key: string): T {
        return JSON.parse(readFileSync(this.storeLocation + "/" + JetStore.hashing(key), "utf8")) as T
    }
}
